...
export const FileUploadForm: React.FC<FileUploadFormProps> = ({
  ...
  const toggleSpecialTag = useCallback((tag: string) => {
  ...
  const executeUpload = useCallback(async () => {
    ...
    try {
      ...
      toast.success(`${fileType === 'team' ? 'チーム' : 'マッチ'}データがアップロードされました`);
    } catch (error: any) {
      if (error?.errors) {
      ...
      >
        ...
        >
          ...
          <div
            style={{
              display: 'grid',
              gridTemplateColumns: fileType === 'match' ? 'repeat(auto-fit, minmax(200px, 1fr))' : 'repeat(auto-fit, minmax(250px, 1fr))',
              gap: '20px',
              fontSize: '14px',
            }}
          >
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <input
              ...
              style={{
                ...
                padding: '12px 16px',
                background: '#020824',
                border: fieldErrors.ownerName ? '2px solid #ef4444' : '1px solid #1E3A5F',
                borderRadius: '8px',
                color: '#fff',
                fontSize: '14px',
                outline: 'none',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  onChange={(e) => updateFormData({ deletePassword: e.target.value })}
                  placeholder="削除時に必要なパスワードを設定"
                  style={{
                    width: '100%',
                    padding: '12px 50px 12px 16px',
                    background: '#020824',
                    border: fieldErrors[`${fileType}DeletePassWord`] ? '2px solid #ef4444' : '1px solid #1E3A5F',
                    borderRadius: '8px',
                    color: '#fff',
                    fontSize: '14px',
                    outline: 'none',
              ...
              <p style={{ fontSize: '12px', color: '#8CB4FF', margin: 0 }}>
                このパスワードは{fileType === 'team' ? 'チーム' : 'マッチ'}データを削除する際に必要です。
              </p>
            </div>
          )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <div
              style={{
                border: `2px dashed ${
                  fieldErrors.file ? '#ef4444' :
                  selectedFile ? '#00c8ff' :
                  dragActive ? '#00c8ff' : '#1E3A5F'
                }`,
                borderRadius: '12px',
                padding: '32px',
                textAlign: 'center',
                cursor: 'pointer',
                transition: 'all 0.2s',
                background: dragActive || selectedFile ? 'rgba(0, 200, 255, 0.05)' : 'transparent',
              }}
              ...
              onMouseEnter={(e) => {
                if (!selectedFile && !dragActive && !fieldErrors.file) {
                  ...
                  (e.target as HTMLDivElement).style.background = 'rgba(0, 200, 255, 0.05)';
                }
              }}
              onMouseLeave={(e) => {
                if (!selectedFile && !dragActive && !fieldErrors.file) {
            ...
            >
              {selectedFile ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', alignItems: 'center' }}>
                  ...
                  <div>
                    <p style={{ fontWeight: '600', color: '#00c8ff', margin: '0 0 4px 0' }}>
                      {selectedFile.name}
                    </p>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', alignItems: 'center' }}>
                  ...
                  <div>
                    <p style={{ fontWeight: '600', color: '#b0c4d8', margin: '0 0 4px 0' }}>
                      CHEファイルをドラッグ&ドロップ
                    </p>
                    <p style={{ fontSize: '14px', color: '#8CB4FF', margin: 0 }}>
                      またはクリックしてファイルを選択
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: '#8CB4FF' }}>
              <span>対応形式: .CHE</span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            />
            <p id="datetime-help" style={{ fontSize: '12px', color: '#8CB4FF', margin: 0 }}>
        ...
        {showConfirmDialog && (
          ...
          >
            ...
            >
              ...
              >
                以下の内容で{fileType === 'team' ? 'チーム' : 'マッチ'}データをアップロードします。
              </p>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '14px' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <span style={{ color: '#b0c4d8' }}>
                      {selectedFile ? `${(selectedFile.size / 1024).toFixed(2)} KB` : '0 KB'}
                    </span>
                  </div>
                  ...
                  {formData.downloadDate && (
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ color: '#8CB4FF' }}>ダウンロード可能日時:</span>
                      <span style={{ color: '#b0c4d8' }}>
