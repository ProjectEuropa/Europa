...
if [ "${RUN_CRON:-0}" = "1" ]; then
  ...
  scp -i /root/.ssh/id_rsa -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${BACKUP_DIR}/${FILE_NAME} ssh.from.production:/home/project.europa/Europa/backups
fi
